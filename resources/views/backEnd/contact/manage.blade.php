        ...
        <div class="col-12">
          <div class="card">
            ...
            <h3 class="card-title">Manage contact-us information</h3>
              ...
              <a href="{{url('editor/contact-us/add')}}"><i class="fa fa-plus"></i>Add</a>
      			</div>
          </div>
          <!-- /.card-header -->
                ...
                <tr>
                  ...
                  <td>
                    <ul class="action_buttons dropdown">
                        ...
                        <li>
                        @if($value->status==1)
                        <form action="{{url('editor/contact-us/unpublished')}}" method="POST">
                          @csrf
                          <input type="hidden" name="hidden_id" value="{{$value->id}}">
                          <button type="submit" class="thumbs_up"><i class="fa fa-thumbs-up"></i> Active</button>
                        </form>
                        @else
                          <form action="{{url('editor/contact-us/published')}}" method="POST">
                            @csrf
                            <input type="hidden" name="hidden_id" value="{{$value->id}}">
                      ...
                      <li>
                        <a class="edit_icon" href="{{url('editor/contact-us/edit/'.$value->id)}}"><i class="fa fa-edit"></i> Edit</a>
                      </li>
                      <li>
                        <form action="{{url('editor/contact-us/delete')}}" method="POST">
                          @csrf
                          <input type="hidden" name="hidden_id" value="{{$value->id}}">
