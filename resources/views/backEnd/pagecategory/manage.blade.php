        ...
        <div class="col-12">
          <div class="card">
            ...
            <div class="short_button">
              <a href="{{url('editor/pagecategory/create')}}"><i class="fa fa-plus"></i>Add</a>
            </div>
          </div>
          <!-- /.card-header -->
                  ...
                  <td>
                   <ul class="action_buttons dropdown">
                        ...
                        <ul class="dropdown-menu">
                          <li>
                        @if($value->status==1)
                        <form action="{{url('editor/pagecategory/inactive')}}" method="POST">
                          @csrf
                          <input type="hidden" name="hidden_id" value="{{$value->id}}">
                          <button type="submit" class="thumbs_up" title="unpublished"><i class="fa fa-thumbs-up"></i> Active</button> 
                        </form>
                        @else
                          <form action="{{url('editor/pagecategory/active')}}" method="POST">
                            @csrf
                            <input type="hidden" name="hidden_id" value="{{$value->id}}">
                        ...
                        @endif
                          ...
                          <li>
                        <a class="edit_icon" href="{{url('editor/pagecategory/edit/'.$value->id)}}" title="Edit"><i class="fa fa-edit"></i> Edit</a>
                      </li>
                        </ul>
                    </ul>
                  </td>
