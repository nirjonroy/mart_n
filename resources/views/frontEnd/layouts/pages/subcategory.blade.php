        ...
        <div class="col-sm-12">
          <div class="custom-breadcrumb">
            <ul>
              ...
              <li><a class="anchor"><p>{{$subcategory->subcategoryName}}</a> - Total Found {{$totalprocuct}} Products</p></li>
            </ul>
          </div>
        </div>
      </div>
          ...
          <div class="sidebar">
              <div class="custom-sidebar ">
                <div class="title ">
                 ...
                 <div class="category-checkbox @if($childcategories->count() > 10) scrollClass  @endif">
                   @foreach($childcategories as $key=>$value)
                      <div class="sidebar-item row">
                        @php
                            $totalproduct = App\Product::where(['status'=>1,'productchildcat'=>$value->id])->count();
                        @endphp
                        @if($totalproduct)
                         <div class="col-lg-9 col-md-9 col-sm-9">
                             <div class="ls-checkbox">
                              <label class="cat-chechbox"><p>{{$value->childcategoryName}}</p>
                                <input type="checkbox" value="{{$value->id}}" name="category[]" class="subcategory">
                         ...
                         @foreach($colors as $color)
                            <div class="sidebar-item row">
                                @php
                                 ...
                                 ->where(['products.status'=>1,'products.productsubcat'=>$subcategory->id,'color_id'=>$color->id])
                                 ->count();
                                @endphp
                                @if($allcolor)
                               <div class="col-lg-9 col-md-9 col-sm-9">
                      ...
                      <div class="title ">
                         ...
                         <div class="category-checkbox @if($brands->count() > 10) scrollClass  @endif">
                          @foreach($brands as $brand)
                              @php
                                $totalproduct = App\Product::where(['status'=>1,'request'=>1,'productbrand'=>$brand->id,'productsubcat'=>$subcategory->id])->count();
                              @endphp
                ...
                <div class="title ">
                  ...
                  <div class="category-checkbox">
                    ...
                    <div class="sidebar-item row">
                        @php
                            $var2=$var1+$var1;
                            $priceproduct = App\Product::where(['productsubcat'=>$subcategory->id,'request'=>1,'status'=>1])->whereBetween('productnewprice',[$var1,$var2])->get();
                        @endphp
                        @if($priceproduct)
                          <div class="col-lg-9 col-md-9 col-sm-9">
                               <div class="ls-checkbox ">
      ...
      <div class="v-tab_container">
        <div id="category" class="v-tab_content">
          <ul>
            @foreach($childcategories as $key=>$value)
            <li>
               <a href="{{url('/products/'.$value->slug.'/'.$value->id)}}">{{$value->childcategoryName}}
                @php
                    $totalproduct = App\Product::where(['status'=>1,'productchildcat'=>$value->id])->get();
                  @endphp
        ...
        <div id="color" class="v-tab_content">
          <ul>
            @foreach($colors as $color)
              <li>
                 <a href="{{url('subcat/color/'.$color->slug.'/'.$subcategory->id.'/'.$color->id)}}">{{$color->colorName}}
                  @php
                   $allcolor =\App\Productcolor::join('colors','productcolors.color_id','=', 'colors.id')
                      ->join('products','productcolors.product_id','=', 'products.id')
                     ->select('productcolors.*')
        ...
        <div id="brand" class="v-tab_content">
          <ul>
            @foreach($brands as $brand)
                @php
                  $totalproduct = App\Product::where(['status'=>1,'request'=>1,'productbrand'=>$brand->id])->get();
                @endphp
              <li>
                 <a href="{{url('subcat/brand/'.$brand->slug.'/'.$subcategory->id.'/'.$brand->brand_id)}}">{{$brand->brandName}}
                  <span>({{$totalproduct->count()}})</span>
                  </a>
              </li> 
            @endforeach
        ...
        <div id="pricerange" class="v-tab_content">
           <ul>
            ...
            @endphp
              <li>
                 <a href="{{url('/subcat/price/'.$subcatid.'/'.$var1.'/'.$var2)}}">{{$var1}} -  {{$var2}}
        ...
        <div id="discount" class="v-tab_content">
            <ul>
               @for($dis=10; $dis <= 80; $dis=$dis+10)
                  ...
                  @endphp
                  <li>
                     <a href="{{url('subcat/discount/'.$subcat_id.'/'.$dis)}}">{{$dis}}% and Above
                      </a>
                  </li> 
                 @endfor
            </ul>
...
<script>
      $('.dynamic_select').on('change', function () {
          var mainroute = $('.mainroute').val(); // get selected value
          var slug = $('.slug').val(); // get selected value
          var id = $('.id').val(); // get selected value
          var sort = $(this).val(); // get selected value
          if (mainroute,slug,id,sort) { 
             window.location='https://gatewaygroup.com.bd/ongona/'+mainroute+'/'+slug+'/'+id+'/'+sort;
          }
          return false;
      });
</script>
